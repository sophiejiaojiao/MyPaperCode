...
public class CalculateScoreAlgorithm {
            ...
            if (post_completeInfo.postTypeId.equals("1")) { // i.e., this post
                ...
                for (int j = 0; j < tags.length; j++) {
                    if (textIsMatchedWithTagOrNot(issTI.projectLanguage,
                            ...
                            tags[j])) {
                        //
                        int scoreOfThePostPlusOne = Integer
                                .parseInt(post_completeInfo.Score) + 1;
                        if (scoreOfThePostPlusOne < 1)
                            // 保证每个question至少有一个匹配的tag（论文有描述，第7页记号笔）
                            scoreOfThePostPlusOne = 1;
                        // 计算每个数的倒数和
                        sumOfInvertedUpVotes = sumOfInvertedUpVotes + 1.0
                                / scoreOfThePostPlusOne;
                        count++;
                        break;
                    }
                }// for.
    ...
    public static void calculateScores(IssueTextualInformation issTI,
        ...
        // soPostsOfThisUserAL表示某成员的所有post
...
//                            / (tempScoreOfThePost * (1 / (timeOfTag + 1)) + 1.0);
            ...
            // System.out.println("SOId: " + cm.SOId + "\t" + "QS: " +
            // Constants.floatFormatter.format(cm.intersection_Q_score) +
            // "\t\tAS: " +
            // Constants.floatFormatter.format(cm.intersection_A_score) +
            // "\t\tSSA Z-score: " +
            // Constants.floatFormatter.format(cm.intersection_z_score));
        }// if.
                ...
                if (ownerLoginAndProjectName_and_ItsCommunityMembers
                    ...
                    for (int p = 0; p < issAL.size(); p++) {
                        ...
                        if (communityMemberThatHasBeenAssignedToThisIssue != null) {// Triage:
                            ProvideData
                                    .initializeIntersectionWithBugsScores(cmAL);
                            ...
                            double NF = calculateHarmonicMeanOfUpVotesForQuestionsTaggedWithAtLeastOneOfTagsInMatchedTags_SO_b(
                                    issTI, posts1ById);
                            // Iterating over all community members, and
                            // calculating their scores for this issue:
                            // 循环该项目的每一个成员
                            for (int k = 0; k < cmAL.size(); k++) {
    ...
    public static void main(String[] args) {
        ...
        // for (int i=0; i<1; i++)
        // triage2_basedOnMultipleCriteria(Constants.DATASET_DIRECTORY_DATASETS_FOR_TRIAGING,
        // "communitiesOf17Projects.tsv", "communitiesSummary(17Projects).tsv",
        ...
        // 20,Constants.DATASET_DIRECTORY_TRIAGE_RESULTS, "17Projects.tsv",
        ...
        for (int i = 0; i < 1; i++)
            triage2_basedOnMultipleCriteria(
                    ...
                    "testFor3Projects.tsv",// 3Projects.tsv"
                    100000, Constants.THIS_IS_REAL);
