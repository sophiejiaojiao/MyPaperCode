...
public class TSVManipulations {
    ...
    public static TreeMap<String, String[]> readUniqueKeyAndItsValueFromTSV(String inputTSVPath, String inputTSVFileName, Set<String> keySetToCheckExistenceOfKeyField,
        ...
        try {
            ...
            while ((s = br.readLine()) != null) {
                ...
                if (fields.length == totalFieldsCount) {//totalFieldsCount=9
                        ...
                        if (keySetToCheckExistenceOfKeyField == null || keySetToCheckExistenceOfKeyField.contains(keyField)) {
                            ...
                            if (!tsvRecordsHashMap.containsKey(keyField)) {
                                if (fieldNumbersToBeRead_separatedByDollar.equals("ALL"))//means that all the fields are needed.
                                    tsvRecordsHashMap.put(keyField, fields);//将整个fields的内容都存放到了tsvRecordsHashMap中，其对应键值为keyField
                                else {//means that only some of the fields are needed.
                                    String[] neededFields = fieldNumbersToBeRead_separatedByDollar.split("\\$");//"0$1$2$3$4$5"
                                    for (int k = 0; k < neededFields.length; k++)
                                        neededFields[k] = fields[Integer.parseInt(neededFields[k])];//Integer.parseInt将String字符类型数据转换为Integer整型数据
                ...
                if (testOrReal > Constants.THIS_IS_REAL)
                    if (i >= testOrReal)
                        break;
                if (i % showProgressInterval == 0)//取余为1
                    System.out.println("        " + Constants.integerFormatter.format(i));
            }//while ((s=br....
            if (error1 > 0)
    ...
    public static HashSet<String> readUniqueFieldFromTSV(String inputTSVPath, String inputTSVFileName,
        ...
        //该方法目的是找到communitiesSummary(20/3Projects).tsv文件中的唯一键值项，如果有重复的，则增加error值，最后报告出来
        HashSet<String> tsvfieldHashSet = new HashSet<String>();
        try {
            BufferedReader br;
            //Reading posts and adding <repoId, totalNumberOfMembers> in a hashMap:
            ...
            while ((s = br.readLine()) != null) {
                fields = s.split("\t");
                if (fields.length != totalFieldsCount)//totalFieldsCount=3
                    error1++;
                else {
                    //recordShouldBeRead值为true
                    recordShouldBeRead = MyUtils.runLogicalComparison(conditionType, fields[field1Number], field1Value, fields[field2Number], field2Value);
                    if (recordShouldBeRead) {
                        keyField = fields[keyFieldNumber];//keyFieldNumber=1，即项目名
                        //将tsv文件中的内容读取到tsvfieldHashSet中，如果tsvfieldHashSet不包含该项目名，则添加；如果已经存在，在不添加，error值加1;该操作是为了保证tsvfieldHashSet中的项目名唯一
                        if (!tsvfieldHashSet.contains(keyField))
                            tsvfieldHashSet.add(keyField);//只向tsvfieldHashSet添加了keyField，即项目名
                        else
                            error2++;
                        matchedRec++;
                    }//if (reco....
                ...
                i++;//tsv文件中的内容读取完成之后，i自增1
                if (testOrReal > Constants.THIS_IS_REAL)//THIS_IS_REAL=-1=testOrReal
                    if (i >= testOrReal)
                        break;
                if (i % showProgressInterval == 0)//i一定会自增1为1，那么取余则为1，肯定不可能为0啊，为什么要这样判断呢？
                    System.out.println("        " + Constants.integerFormatter.format(i));
            }//while ((s=br....
            if (error1 > 0)
                System.out.println("        Error) Number of records with != " + totalFieldsCount + " fields: " + error1);
    ...
    public static TreeMap<String, ArrayList<String[]>> readNonUniqueKeyAndItsValueFromTSV(String inputTSVPath, String inputTSVFileName, Set<String> keySetToCheckExistenceOfKeyField,
            ...
            while ((s = br.readLine()) != null) {
                ...
                if (fields.length != totalFieldsCount)
                    ...
                    if (recordShouldBeRead) {
                        ...
                        if (keySetToCheckExistenceOfKeyField == null || keySetToCheckExistenceOfKeyField.contains(keyField)) {
                            if (tsvRecordsHashMap.containsKey(keyField)) {
                                aTSVItemRelatedToTheRepeatedKey = tsvRecordsHashMap.get(keyField);//通过keyField这个键，获取其对应的一行值
                                isNew = true;
                                //以下for循环应该是查找是否存在相同内容的两行数据
                                for (String[] stringArray : aTSVItemRelatedToTheRepeatedKey)
                                    //fields表示读入的这行数据，stringArray表示已添加的、与新行keyField相同的数据；返回true表示两行数据完全相等
                                    //但是因为已有行长度为5，新行长度为9，所以此处永远是false
                                ...
                                if (isNew) {//两行数据并不是完全相同，只是至少keyField相同
                                    if (fieldNumbersToBeRead_separatedByDollar.equals("ALL"))//means that all the fields are needed.
                                        ...
                                        aTSVItemRelatedToTheRepeatedKey.add(neededFields);//后续未对aTSVItemRelatedToTheRepeatedKey进行处理，那么添加得到这个hashmap有什么用呢？
                                    }//else.
                                }//if
                            }//if.tsvRecordsHashMap.containsKey(keyField)
                            else {//添加新的OwnerUserId值
                                ...
                                if (fieldNumbersToBeRead_separatedByDollar.equals("ALL"))//means that all the fields are needed.
                                    ...
                                    for (int k = 0; k < neededFields.length; k++)
                                        neededFields[k] = fields[Integer.parseInt(neededFields[k])];
                                    aTSVItemRelatedToANewKey.add(neededFields);//将0$1$2$3$6对应的值添加到aTSVItemRelatedToANewKey，局部变量
                                }//else.
                                tsvRecordsHashMap.put(keyField, aTSVItemRelatedToANewKey);//以keyField作为键，存入一行值
                            }//else.
                            matchedRec++;
                        }//if (tsvRecordsHashMap.containsKey(keyField)).
                    }//if (reco....
        ...
        return tsvRecordsHashMap;//处理posts时，tsvRecordsHashMap只存有OwnerUserId键值唯一的61876条数据，posts总数据为223801
    }
            ...
            while ((s = br.readLine()) != null) {
                ...
                if (fields.length == 9) {
                    ...
                    if (allSOIdsOfCommunityMembers.contains(ownerId))
                        ...
                        else if (fields[1].equals("2")) {//i.e., this is an answer. So we need both the answer and the related question:
